...
raw.data <- 
  read.csv("Census Custom Over 18_Clean.csv") %>%
...
# For each demographic grouping (except those 1-11 years old), select the mean amount of bias
...
# Assuming underreporting is the same for meth as for cocaine
...
for (i in 1:nrow(census.drug)){
  census.drug$MJ.TRUE[i] <-
    ifelse(is.na(census.drug$MRJMON[i]), census.drug$value[i],
           ifelse(census.drug$MRJMON[i]==0, round(census.drug$MJ.SELFREPORT[i]*(1-census.drug$MJ.BIAS[i])),
           round(census.drug$MJ.SELFREPORT[i] + (census.drug$MJ.SELFREPORT[i-1]*census.drug$MJ.BIAS[i]))))

  census.drug$COC.TRUE[i] <-
    ifelse(is.na(census.drug$COCMON[i]), census.drug$value[i],
          ifelse(census.drug$COCMON[i]==0, round(census.drug$COC.SELFREPORT[i]*(1-census.drug$COC.BIAS[i])),
           round(census.drug$COC.SELFREPORT[i] + (census.drug$COC.SELFREPORT[i-1]*census.drug$COC.BIAS[i]))))
  
  census.drug$METH.TRUE[i] <-
    ifelse(is.na(census.drug$METHAMMON[i]), census.drug$value[i], 
          ifelse(census.drug$METHAMMON[i]==0, round(census.drug$METH.SELFREPORT[i]*(1-census.drug$COC.BIAS[i])),
           round(census.drug$METH.SELFREPORT[i] + (census.drug$METH.SELFREPORT[i-1]*census.drug$COC.BIAS[i]))))
}
...
true.data.meth <- 
  uncount(census.drug, METH.TRUE) %>%
  rowid_to_column() %>%
  mutate(METH.TRUE = ifelse(METHAMMON == 0, 0, 1)) %>%
  select(rowid, METH.TRUE)
...
bias.data.meth <- 
  uncount(census.drug, METH.SELFREPORT) %>%
  rowid_to_column() %>%
  mutate(METH.SELFREPORT = ifelse(METHAMMON == 0, 0, 1)) %>%
  select(rowid, METH.SELFREPORT)

# Join dataframes and limit to n = 100,000
sim.data.final <- 
  left_join(true.data.mj, true.data.coc, by = "rowid") %>%
  left_join(., true.data.meth, by="rowid") %>%
  left_join(., bias.data.mj, by = "rowid") %>%
  left_join(., bias.data.coc, by = "rowid") %>%
  left_join(., bias.data.meth, by = "rowid") %>%
  sample_n(100000)
...
save(meth.pop, file = "/Users/nslevy/Documents/Columbia PhD/Misclassification of Drug Use/meth.pop.RData")
...
mj.n <- data.frame(prop.table(svytable(~MRJMON, subset(dnsduh, CATAG3>1)))) %>% rename(MJ = MRJMON, NSDUH = Freq)
mj.sim <- data.frame(prop.table(table(sim.data.final$MJ.SELFREPORT))) %>% rename(MJ = Var1, SIMULATED = Freq)
coc.n <- data.frame(prop.table(svytable(~COCMON, subset(dnsduh, CATAG3>1)))) %>% rename(COC = COCMON, NSDUH = Freq)
coc.sim <- data.frame(prop.table(table(sim.data.final$COC.SELFREPORT))) %>% rename(COC = Var1, SIMULATED = Freq)
meth.n <- data.frame(prop.table(svytable(~METHAMMON, subset(dnsduh, CATAG3>1)))) %>% rename(METH = METHAMMON, NSDUH = Freq)
meth.sim <- data.frame(prop.table(table(sim.data.final$METH.SELFREPORT))) %>% rename(METH = Var1, SIMULATED = Freq)

table1_drugs <- cbind(mj.n, mj.sim, coc.n, coc.sim, meth.n, meth.sim)
...
# Meth
methsr <- 
  as.data.frame(prop.table(table(sim.data.final$METH.SELFREPORT))) %>%
  rename(SelfReport = Freq,
         Drug = Var1)
methtrue <- 
  as.data.frame(prop.table(table(sim.data.final$METH.TRUE))) %>%
  rename(Truth = Freq,
         Drug = Var1)

methresults <- 
  left_join(methsr, methtrue, by="Drug") %>%
  mutate(Drug = ifelse(Drug==1,"Meth",""))

methresults <- methresults[2,]

drugresults <- rbind(mjresults, cocresults, methresults) %>%
...
totalresults <- pivot_longer(drugresults,
                             c(SelfReport, Truth),
                             names_to = "Type") %>%
  mutate(Drug = factor(Drug, levels = c("MJ", "Cocaine", "Meth")),
...
selfreport.coc <-
  ...
  group_by(DEMFLAG) %>%
  count(DEMFLAG) %>%
  rename(COC.SELFREPORT = n)

selfreport.meth <-
  sim.study %>%
  filter(METH.SELFREPORT == 1) %>%
  group_by(DEMFLAG) %>%
  count(DEMFLAG) %>%
  rename(METH.SELFREPORT = n)
...
sim.selfreport <- left_join(sim.selfreport, selfreport.meth, by = "DEMFLAG")
...
sim.true <- left_join(sim.true, true.meth, by = "DEMFLAG")
...
sim.corrected <-
  ...
  mutate(MJ.SENS = MJ.SELFREPORT/MJ.TRUE,
         ...
         METH.SENS = METH.SELFREPORT/METH.TRUE)

sim.corrected <-
  sim.corrected %>%
  mutate(MJ.CORRECTED = MJ.SELFREPORT/MJ.SENS,
         COC.CORRECTED = COC.SELFREPORT/COC.SENS,
         METH.CORRECTED = METH.SELFREPORT/METH.SENS)

sim.corrected[sim.corrected == 0.1] <- 0

corrected.data <- sim.corrected[ , c(-1, -6:-11)]
corrected.counts <- summarise_all(corrected.data, sum)

corrected.samp <- 
  corrected.counts %>%
  pivot_longer(MJ.SELFREPORT:METH.CORRECTED) %>%
  mutate(Drug = factor(rep(c("MJ","Cocaine","Meth"), 2), levels = c("MJ","Cocaine","Meth")),
         Type = factor(rep(c("SelfReport","Corrected"), each = 3), 
...
corrected.results.sum <-
  corrected.results %>%
  group_by(Drug, Type) %>%
  summarise(P25 = quantile(Prev, 0.25),
            P75 = quantile(Prev, 0.75),
            Prev = quantile(Prev, 0.5),
            Mean = mean(Prev), 
            .groups = "keep")

corrected.results.final <- bind_rows(corrected.results.sum, true.props) %>%
  arrange(Type)

corrected.results.combined <- left_join(corrected.samp.final, corrected.results.sum, by = c("Drug","Type")) %>%
  rename(Prev = Prev.x,
         Median = Prev.y)

write.csv(corrected.results.combined, file = "Simple Random Sample Results.csv", row.names = FALSE)

(correctedresults.plot <- 
    ggplot(data = corrected.results.final,
           aes(x = Drug, y = Prev, fill = Type)) +
    geom_bar(stat = "identity", position = position_dodge()) +
    geom_errorbar(aes(ymin = P25, ymax = P75), position = position_dodge2(width = 0.5, padding = 0.5), size=0.3) +
    scale_fill_manual(values = c("#453781FF","#1F968BFF","#B8DE29FF")) +
    theme(axis.title.y = element_blank(),
          axis.title.x = element_blank(),
          panel.grid.major = element_blank(), 
...
save_plot(filename = "Simulated SRS Correction with IQR.pdf", plot = correctedresults.plot)
...
samp3 <- 
  add_count(samp3, DEMFLAG, wt = n(), name = "TOTAL") %>%
...
sim.selfreport.stratified <- left_join(sim.selfreport.stratified, selfreport.meth, by = "DEMFLAG")
...
sim.true.stratified <- left_join(sim.true.stratified, true.meth, by = "DEMFLAG")
...
corrected.data <- sim.corrected.stratified[ , c(-1, -6:-11)] * (1/sim.selection.prob$selection.prob)
...
corrected.results.stratified <- bind_rows(corrected.res.strat)

corrected.results.str.sum <-
  corrected.results.stratified %>%
  group_by(Drug, Type) %>%
  summarise(P25 = quantile(Prev, 0.25),
            P75 = quantile(Prev, 0.75),
            Prev = quantile(Prev, 0.5),
...
corrected.results.strat.final <- bind_rows(corrected.results.str.sum, true.props)

corrected.results.strat.combined <- left_join(corrected.strat.final, corrected.results.str.sum, by = c("Drug","Type")) %>%
  rename(Prev = Prev.x,
         Median = Prev.y)

write.csv(corrected.results.strat.combined, "Stratified Random Sample Results.csv")
...
srs.func <- function(x){
  ...
  estimates <- 
    summarise(df, n = nrow(df), MJ.EST = sum(MJ.TRUE)/nrow(df), COC.EST = sum(COC.TRUE)/nrow(df), METH.EST = sum(METH.TRUE)/nrow(df))
  ...
  estimates$METH.DIFF = abs(estimates$METH.EST - as.numeric(true.props[3,3]))
  return(estimates)
}
...
save_plot(filename = "Meth Power by Sample Size.pdf", plot = meth.power.plot)

power.plots <- plot_grid(mj.power.plot, coc.power.plot, meth.power.plot, align = "h", nrow = 1)
...
meth.pop2 <- filter(meth.pop, METHAMMON == 1) %>%
  rename(METH.SELFREPORT = Freq) %>%
  select(-METHAMMON)

# Sensitivities based on simple random sample
sensitivity <- 
  sim.corrected %>%
  select(DEMFLAG, MJ.SENS, COC.SENS, METH.SENS)

nsduh.corrected <- left_join(nsduh.pop2, mj.pop2, by = "DEMFLAG2")
nsduh.corrected <- left_join(nsduh.corrected, coc.pop2, by = "DEMFLAG2")
nsduh.corrected <- left_join(nsduh.corrected, meth.pop2, by = "DEMFLAG2")
...
nsduh.corrected.str <- left_join(nsduh.corrected.str, meth.pop2, by = "DEMFLAG2")
nsduh.corrected.str <- left_join(nsduh.corrected.str, sensitivity.strat, by = c("DEMFLAG2" = "DEMFLAG"))

# NOTE: In simple random sample, we didn't sample every demographic combo in our study data so setting missing values to 1
# Assuming perfect sensitivity since we don't know
...
nsduh.corrected <-
  ...
  mutate(MJ.SENS = if_else(is.na(MJ.SENS), 1, if_else(MJ.SENS == 0, 0.1, MJ.SENS)),
         ...
         METH.SENS = if_else(is.na(METH.SENS), 1, if_else(METH.SENS == 0, 0.1, METH.SENS)),
         MJ.CORRECTED = MJ.SELFREPORT/MJ.SENS,
         COC.CORRECTED = COC.SELFREPORT/COC.SENS,
         METH.CORRECTED = METH.SELFREPORT/METH.SENS)

nsduh.corrected2 <- nsduh.corrected[ , c(-1, -6:-8)]
...
nsduh.corrected.str2 <- nsduh.corrected.str[ , c(-1, -6:-8)]
